...
def evaluate_model(valid_dataloader, train_dataloader, nll_per_action, model, epoch_key=None):
    ...
    nll_per_action = valid_nll_list + train_nll_list
...
def get_validation_nll1(dataloader, model):
        """
        ...
        n_samples    = min(100000, C.n_samples)  # n graphs to evaluate
        nlls  = torch.zeros(n_samples * (C.max_n_nodes+5),
                                   device="cuda")
        ...
        for idx, batch in enumerate(dataloader):
            ...
            start_idx  = idx * C.batch_size
            end_idx    = idx * C.batch_size + len(nll)
            nlls[start_idx:end_idx] = nll
        ...
        avg_final_likelihood = torch.sum(nlls, dim=0) / n_structures[0]

        return nlls, avg_final_likelihood
