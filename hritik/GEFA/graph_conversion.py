...
def aa_sas_feature(target, dataset='davis'):
    feature = []
    file = config.data_path +dataset+'/profile/' + target + '_PROP/' + target + '.acc'
...
def aa_ss_feature(target, dataset='davis'):
    feature = []
    file = config.data_path+dataset + '/profile/' + target + '_PROP/' + target + '.ss8'
    for line in open(file):
        cols = line.strip().split()
        if len(cols) == 11:
            res_sas = []
...
def prot_to_graph(seq, prot_contactmap, prot_target, file_, plot_, dataset='davis'):
    ...
    sequence_output = np.load(config.data_path +dataset+ '/emb/' + prot_target + '.npz', allow_pickle=True)
