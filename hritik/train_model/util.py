...
def get_last_epoch():
    ...
    convergence_path = constants.job_dir + "convergence.csv"
...
def get_last_epoch1() -> str:
    """
    Gets previous training epoch by reading it from the "convergence.csv" file.
...
def normalize_evaluation_metrics1(property_histograms : dict,
...
def get_restart_epoch() -> Union[int, str]:
    ...
    if constants.restart or constants.job_type == "test":
        ...
        generation_path = constants.job_dir + "generation.csv"
        epoch           = "NA"
        row             = -1
...
def load_ts_properties1(csv_path : str) -> dict:
    """
...
def write_last_molecule_idx1(last_molecule_idx : int, dataset_size : int,
...
def write_graphs_to_smi1(smi_filename : str,
...
def write_model_status(epoch=None, lr=None, loss=None, score=None, append=True):
    ...
    if not append:  # create the file
        ...
        if constants.job_type == "train":  # only write a `convergence.csv` when training
            if score is None:
...
def write_training_status(epoch : Union[int, None]=None,
    ...
    if not append:  # create the file
        ...
        # only write a `convergence.csv` when training
...
def write_molecules1(molecules : list,
...
def write_ts_properties1(training_set_properties : dict) -> None:
    """
...
def write_validation_scores1(output_dir : str, epoch_key : str,
