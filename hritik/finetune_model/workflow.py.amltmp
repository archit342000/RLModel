...
class Workflow:
    ...
    def define_model_and_optimizer(self):
        ...
        if self.C.restart:
            ...
            self.agent_model = torch.load(f"{model_dir}model_restart_{self.restart_epoch}.pth")
            self.prior_model = torch.load(f"{model_dir}model_restart_{self.restart_epoch}.pth")
            self.prev_model = torch.load(f"{model_dir}model_restart_{self.restart_epoch}.pth")
            ...
            self.agent_model = torch.load(f"{model_dir}model_restart_{self.C.epochs}.pth")
            self.prior_model = torch.load(f"{model_dir}model_restart_{self.C.epochs}.pth")
            self.prev_model = torch.load(f"{model_dir}model_restart_{self.C.epochs}.pth")
            with open(self.C.data_path + "qsar_model.pickle", 'rb') as file:
                model_dict = pickle.load(file)                                      
                self.drd2_model = model_dict["classifier_sv"]
    ...
    def generation_phase(self):
        ...
        model_dir = self.C.dataset_dir
        self.prior_model = torch.load(f"{model_dir}model_restart_{self.C.epoch}.pth")
